(function(){
  ...
  function VariableButtons({event,targetValue,onChange}){
    if(!event){return null}
    return h(Space,{size:[6,6],wrap:true,style:{marginTop:6}},event.variables.map(variable => h(Button,{key:variable,size:'small',onClick:() => onChange((targetValue || '') + `{${variable}}`)},`插入${variable}`)));
  }
  ...
  function NotificationConfigEditor(){
    ...
    const [initiator,setInitiator] = React.useState(Boolean(initial.recipients && initial.recipients.initiator));
    ...
    const appendVariable = (value,setValue,variable) => setValue((value || '') + `{${variable}}`);
    const insertControls = (value,setValue) => h(VariableButtons,{event,targetValue:value,onChange:setValue});
    const smsTemplates = [
    ...
    const channelCards = [
      h(ChannelCard,{key:'station',channel:'station',title:'站内通知'},h(Fragment,null,
        ...
        h(Form.Item,{label:'通知正文'},h(Fragment,null,h(Input.TextArea,{value:stationBody,disabled:viewOnly,placeholder:'请输入通知正文',autoSize:{minRows:4,maxRows:8},onChange:event => {setStationBody(event.target.value);setMethodError('')}}),insertControls(stationBody,setStationBody)))
      )),
      ...
      h(ChannelCard,{key:'email',channel:'email',title:'邮件'},h(Fragment,null,
        ...
        h(Form.Item,{label:'邮件正文'},h(Fragment,null,h(Input.TextArea,{value:emailBody,disabled:viewOnly,placeholder:'请输入邮件正文',autoSize:{minRows:4,maxRows:8},onChange:event => {setEmailBody(event.target.value);setMethodError('')}}),insertControls(emailBody,setEmailBody)))
      )),
      h(ChannelCard,{key:'wechat',channel:'wechat',title:'企业微信'},h(Fragment,null,
        ...
        h(Form.Item,{label:'消息正文'},h(Fragment,null,h(Input.TextArea,{value:wechatBody,disabled:viewOnly,placeholder:'请输入消息正文',autoSize:{minRows:4,maxRows:8},onChange:event => {setWechatBody(event.target.value);setMethodError('')}}),insertControls(wechatBody,setWechatBody)))
      ))
    ];
    const targetColumns = [
    ...
    targetColumns.push({title:'操作',key:'action',width:80,align:'center',render:(_,person) => viewOnly || person.isInitiator ? null : h(Button,{type:'link',danger:true,size:'small',onClick:() => setSelectedIds(current => current.filter(id => id !== person.id))},'移除')});
    return h(ConfigProvider,{theme:currentProjectTheme},
      h('div',{className:'notification-react-page',style:configPageStyle()},
        [
          ...
          h(Card,{key:'basic',className:'notification-recipient-card notification-basic-info-card',title:'基本信息',styles:{body:{padding:'20px 24px 24px'}},style:{marginBottom:16,borderRadius:0}},[
            ...
            (targetEnabled || selectedIds.length || (initiator && showIndividualRecipientRows)) ? h(Form.Item,{key:'targets',label:null},[
              targetEnabled ? h(Button,{key:'select',type:'default',className:'notification-target-add',disabled:viewOnly,icon:h('span',{className:'notification-plus-icon','aria-hidden':'true'}),onClick:() => setAccountModalOpen(true)},'选择目标对象') : null,
            ...
            h(Space,{key:'actions',style:{display:'flex',justifyContent:'flex-end'},size:8},[
              h(Button,{key:'cancel',onClick:() => window.backNotificationConfigList()},viewOnly ? '关闭' : '取消'),
              viewOnly ? null : h(Button,{key:'save',type:'primary',htmlType:'submit',onClick:save},'保存配置')
            ])
          ]),
