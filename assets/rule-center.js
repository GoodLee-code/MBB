(function(){
  ...
  const ruleStatuses = ['生效中','待生效','已停用'];
  ...
  const triggerRecords = [
    {type:'流量用量监测',id:'RULE202609180001',name:'月度流量阈值提醒',triggerAt:'2026-09-19 09:28:42',msisdn:'138****2468',iccid:'89860488192540182881',usage:'4,096.00',notificationRecordIndex:0,sent:'已发送'},
    {type:'流量用量监测',id:'RULE202609150003',name:'企业卡用量间隔通知',triggerAt:'2026-09-19 08:16:20',msisdn:'139****7214',iccid:'89860488192540182905',usage:'2,048.72',notificationRecordIndex:1,sent:'已发送'},
    {type:'流量用量监测',id:'RULE202609120006',name:'南区流量高峰提醒',triggerAt:'2026-09-18 23:40:12',msisdn:'136****9182',iccid:'89860488192540182764',usage:'0.00',notificationRecordIndex:2,sent:'发送失败'},
    {type:'流量用量监测',id:'RULE202609080004',name:'套餐流量提前通知',triggerAt:'2026-09-18 18:02:07',msisdn:'137****5061',iccid:'89860488192540182690',usage:'812.40',notificationRecordIndex:0,sent:'已发送'},
    {type:'流量用量监测',id:'RULE202608270002',name:'区域流量高峰提醒',triggerAt:'2026-09-18 16:45:33',msisdn:'150****4638',iccid:'89860488192540182571',usage:'6,240.18',notificationRecordIndex:1,sent:'已发送'}
  ];
  ...
  const rulePagesMarkup = `
    ...
    <section id="ruleTriggerRecordsPage" class="content hidden rule-center-page">
      <div class="system-filter rule-center-filter" id="ruleTriggerFilter">
        ...
        ${selectMarkup('triggerSentStatusFilter','通知发送状态',['已发送','发送失败','发送中'])}
      ...
      <div class="table-card rule-table-wrap"><table class="rule-table trigger-table"><thead><tr><th>触发规则类型</th><th>触发规则ID</th><th>触发规则名称</th><th>触发时间</th><th>MSISDN</th><th>ICCID</th><th>使用量（MB）</th><th>通知记录ID</th><th>通知发送状态</th></tr></thead><tbody id="ruleTriggerRows"></tbody></table></div>
    ...
    <section id="ruleCreatePage" class="content hidden rule-center-page rule-create-page">
      <div class="rule-create-shell">
        ...
        <section class="rule-form-section"><div class="rule-form-title">监测范围</div><div class="rule-radio-row" role="radiogroup" aria-label="监测范围"><label><input type="radio" name="ruleScope" value="all" checked>全平台监测</label><label><input type="radio" name="ruleScope" value="custom">自定义监测</label><label><input type="radio" name="ruleScope" value="range">范围监测</label></div><div class="rule-upload-row hidden" id="ruleScopeUploadRow"><label class="rule-upload-btn">上传文件<input type="file" id="ruleScopeUpload" accept=".csv,.xlsx,.xls"></label><span>支持 CSV、XLSX 文件</span></div><div class="rule-form-grid rule-scope-grid">
          <label class="rule-field"><span class="rule-field-label">供应商</span>${selectMarkup('ruleCreateSupplier','请选择供应商',['中国移动','中国联通','中国电信'])}</label>
          <label class="rule-field"><span class="rule-field-label">商户</span>${selectMarkup('ruleCreateMerchant','请选择商户',['上海智联商贸','杭州云旅科技','苏州星河酒店','深圳远帆科技'])}</label>
          <label class="rule-field"><span class="rule-field-label">运营商</span>${selectMarkup('ruleCreateOperator','请选择运营商',['中国移动','中国联通','中国电信'])}</label>
        </div></section>
        <section class="rule-form-section"><div class="rule-form-title">监测规则</div><div class="rule-form-grid">
          ...
          <div class="rule-field rule-field-wide hidden" id="ruleFollowUpField"><span class="rule-field-label">跟进动作</span><div class="rule-radio-row"><label><input type="radio" name="ruleFollowUp" value="yes" checked>通知并关闭流量数据服务</label><label><input type="radio" name="ruleFollowUp" value="no">仅通知</label></div></div>
          <label class="rule-field rule-field-wide" id="ruleRestoreField"><span class="rule-field-label">次月是否恢复数据服务</span><div class="rule-radio-row"><label><input type="radio" name="ruleRestore" value="yes" checked>次月恢复数据服务</label><label><input type="radio" name="ruleRestore" value="no">不恢复</label></div></label>
        </div></section>
        <section class="rule-form-section"><div class="rule-form-title">生效时间</div><div class="rule-radio-row" role="radiogroup" aria-label="生效时间"><label><input type="radio" name="ruleEffective" value="now" checked>立即生效</label><label><input type="radio" name="ruleEffective" value="scheduled">指定日期生效</label><div class="rule-effective-control hidden" id="ruleEffectiveControl"><div class="rule-month-picker" id="ruleEffectiveMonthPicker"><input class="input rule-effective-input" id="ruleCreateEffectiveDate" placeholder="请选择生效年月" aria-label="指定生效年月" readonly><span class="rule-month-clear" role="button" tabindex="0" aria-label="清除生效年月" title="清除"></span><span class="rule-month-trigger" aria-hidden="true"></span><div class="rule-month-panel"><div class="rule-month-toolbar"><button type="button" class="rule-month-nav" data-month-nav="prev" aria-label="上一年">‹</button><span class="rule-month-year" id="ruleMonthYear"></span><button type="button" class="rule-month-nav" data-month-nav="next" aria-label="下一年">›</button></div><div class="rule-month-grid" id="ruleMonthGrid"></div></div></div><div class="rule-effective-help">默认当月1日0点生效</div></div></div></section>
        <section class="rule-form-section"><div class="rule-form-title">通知配置</div><div class="rule-notification-base">
          <div class="rule-notification-base-row"><span class="rule-field-label">通知对象</span><div><button type="button" class="rule-target-picker" id="ruleSharedTargetPicker" onclick="openRuleNotificationAccountModal('shared')"><span>请选择通知对象</span></button></div></div>
          <div class="rule-notification-target-list-row" id="ruleSharedTargetList"></div>
          <div class="rule-notification-base-row"><span class="rule-field-label">通知时间</span><div><div class="rule-radio-row" role="radiogroup" aria-label="通知时间"><label><input type="radio" name="ruleNotificationTime" value="immediate" checked>规则触发后立即通知</label><label><input type="radio" name="ruleNotificationTime" value="scheduled">指定通知时间</label></div><div class="rule-notification-time-control hidden" id="ruleNotificationTimeControl"><input class="input rule-notification-time-input" id="ruleNotificationTimeInput" type="time" step="60" aria-label="指定通知时间"><span class="rule-effective-help">请选择时、分</span></div></div></div>
          <div class="rule-notification-base-row"><span class="rule-field-label">通知方式</span><div class="rule-check-row" role="group" aria-label="通知方式">${notificationTypes.map((item,index)=>`<label><input type="checkbox" name="ruleNotify" value="${escapeHtml(item)}" ${index === 0 || index === 3 ? 'checked' : ''}>${escapeHtml(item)}</label>`).join('')}</div></div>
        </div><div id="ruleNotificationConfigList" class="rule-notification-config-list"></div></section>
  ...
  style.textContent = `
    ...
    .rule-table .link-action.status-toggle-action{color:#f04438}
    .rule-table .link-action.status-toggle-action:hover{color:#d92d20}
    ...
    .rule-trigger-status{font-weight:500}.rule-trigger-status.triggered{color:#1a9b5a}.rule-trigger-status.untriggered{color:#8b95a5}
    .rule-sent.success{color:#1a9b5a;font-weight:500}.rule-sent.fail{color:#e2473f;font-weight:500}.rule-sent.processing{color:#d78b16;font-weight:500}
    ...
    .rule-detail-upload{display:flex;align-items:center;min-height:32px;padding:0 10px;border:1px solid #e5e7eb;border-radius:4px;background:#f8fafc;color:#4e5969}
    ...
    .rule-notification-card-title{display:flex;align-items:center;gap:8px;margin-bottom:12px;color:#30343b;font-size:13px;font-weight:600}
    ...
    .rule-threshold-reminders{display:flex;flex-direction:column;gap:10px;padding-left:140px;margin-top:-4px}
  ...
  function clearRuleSelect(select){
    ...
    if(select.id === 'triggerRuleTypeFilter' || select.id === 'triggerSentStatusFilter'){renderTriggerRecords();}
    if(select.id === 'ruleCreateReminder'){syncReminderMode('');}
    if(select.classList.contains('rule-notification-select')){renderRuleNotificationTemplatePreview(select);}
  }
  ...
  function sentClass(status){return status === '已发送' ? 'success' : status === '发送失败' ? 'fail' : 'processing';}
  ...
  function renderRuleList(){
    ...
    const rows = ruleRecords.filter(row => matchText(`${row.id}${row.name}`,keyword) && (!type || row.type === type) && (!status || row.status === status) && (!triggerStatus || triggerStatusForRule(row) === triggerStatus));
    const tbody = document.getElementById('ruleListRows');
    if(!tbody){return}
    tbody.innerHTML = rows.length ? rows.map(row => `<tr><td>${escapeHtml(row.id)}</td><td>${escapeHtml(row.name)}</td><td>${escapeHtml(row.type)}</td><td><span class="rule-status ${statusClass(row.status)}">${escapeHtml(row.status)}</span></td><td><span class="rule-trigger-status ${triggerStatusForRule(row) === '已触发' ? 'triggered' : 'untriggered'}">${escapeHtml(triggerStatusForRule(row))}</span></td><td>${escapeHtml(row.effectiveAt)}</td><td>${escapeHtml(latestTriggerTimeForRule(row))}</td><td>${escapeHtml(row.createdAt)}</td><td>${escapeHtml(row.updatedAt)}</td><td>${escapeHtml(row.operator)}</td><td class="sticky-action"><button type="button" class="link-action" onclick="openRuleCreatePage('${escapeHtml(row.id)}')">编辑</button><button type="button" class="link-action" onclick="openRuleDetail('${escapeHtml(row.id)}')">查看</button><button type="button" class="link-action status-toggle-action" onclick="toggleRuleStatus('${escapeHtml(row.id)}')">${row.status === '已停用' ? '启用' : '停用'}</button></td></tr>`).join('') : '<tr><td colspan="11" class="rule-empty">暂无符合条件的规则</td></tr>';
  ...
  function ruleDetailReadonlyField(label,value,extraClass=''){
    ...
    return `<div class="rule-field${extraClass ? ` ${extraClass}` : ''}"><span class="rule-field-label">${escapeHtml(label)}</span><div class="rule-detail-readonly">${escapeHtml(display)}</div></div>`;
  }
  ...
  function ruleDetailSectionsMarkup(row){
    const scopeLabel = row.scopeLabel || '全平台监测';
    const scopeMode = row.scopeMode || (scopeLabel === '范围监测' ? 'range' : scopeLabel === '自定义监测' ? 'custom' : 'all');
    ...
    return `<div class="rule-detail-sections">
      ...
      <section class="rule-form-section"><div class="rule-form-title">监测范围</div>
        ${ruleDetailRadioMarkup([{value:'all',label:'全平台监测'},{value:'custom',label:'自定义监测'},{value:'range',label:'范围监测'}],scopeMode)}
        ${scopeMode === 'custom' ? '<div class="rule-upload-row"><div class="rule-detail-upload">已上传范围文件</div><span>支持 CSV、XLSX 文件</span></div>' : ''}
      ...
      <section class="rule-form-section"><div class="rule-form-title">监测规则</div><div class="rule-form-grid">
        ...
        ${reminder === '阈值提醒' ? `<div class="rule-field rule-field-wide"><span class="rule-field-label">跟进动作</span>${ruleDetailRadioMarkup([{value:'通知并关闭流量数据服务',label:'通知并关闭流量数据服务'},{value:'仅通知',label:'仅通知'}],followUp)}</div>` : ''}
        ${followUp !== '仅通知' ? `<div class="rule-field rule-field-wide"><span class="rule-field-label">次月是否恢复数据服务</span>${ruleDetailRadioMarkup([{value:'次月恢复数据服务',label:'次月恢复数据服务'},{value:'不恢复',label:'不恢复'}],restore)}</div>` : ''}
      </div></section>
      ...
      <section class="rule-form-section"><div class="rule-form-title">通知配置</div><div class="rule-notification-base">
        ...
        <div class="rule-notification-base-row"><span class="rule-field-label">通知时间</span><div>${ruleDetailRadioMarkup([{value:'immediate',label:'规则触发后立即通知'},{value:'scheduled',label:'指定通知时间'}],notificationTime)}${notificationTime !== 'immediate' ? `<div class="rule-notification-time-control"><div class="rule-detail-readonly rule-detail-notification-time">${escapeHtml(notificationTime)}</div><span class="rule-effective-help">请选择时、分</span></div>` : ''}</div></div>
        <div class="rule-notification-base-row"><span class="rule-field-label">通知方式</span>${ruleDetailCheckMarkup(notificationOptions,channels)}</div>
      </div><div class="rule-notification-config-list">${notificationCards || '<div class="rule-empty">请先选择通知方式</div>'}</div></section>
    </div>`;
  }
  ...
  function renderTriggerRecords(){
    ...
    const sent = selectValue('triggerSentStatusFilter');
    ...
    const rows = triggerRecords.filter(row => (!type || row.type === type) && (!sent || row.sent === sent) && matchText(`${row.id}${row.name}`,keyword) && matchText(row.triggerAt,triggerTime) && matchText(`${row.msisdn}${row.iccid}`,identity));
    const tbody = document.getElementById('ruleTriggerRows');
    if(!tbody){return}
    tbody.innerHTML = rows.length ? rows.map((row,index) => `<tr><td>${escapeHtml(row.type)}</td><td>${escapeHtml(row.id)}</td><td>${escapeHtml(row.name)}</td><td>${escapeHtml(row.triggerAt)}</td><td>${escapeHtml(row.msisdn)}</td><td>${escapeHtml(row.iccid)}</td><td>${escapeHtml(row.usage)}</td><td>${escapeHtml(notificationRecordIdForTrigger(row,index))}</td><td><span class="rule-sent ${sentClass(row.sent)}">${escapeHtml(row.sent)}</span></td></tr>`).join('') : '<tr><td colspan="9" class="rule-empty">暂无符合条件的触发记录</td></tr>';
  ...
  function resetTriggerRecords(){
    ...
    resetRuleSelect('triggerSentStatusFilter');
  ...
  function resetCreatePage(){
    ['ruleCreateName','ruleCreateEffectiveDate','ruleNotificationTimeInput'].forEach(id => {const input=document.getElementById(id);if(input){input.value='';}});
    ['ruleCreateType','ruleCreateSupplier','ruleCreateMerchant','ruleCreateOperator','ruleCreateReminder'].forEach(resetRuleSelect);
    document.querySelectorAll('#ruleCreatePage input[type="radio"]').forEach(input => {
      const defaultValue = input.name === 'ruleScope' ? 'all' : input.name === 'ruleEffective' ? 'now' : input.name === 'ruleFollowUp' || input.name === 'ruleRestore' ? 'yes' : input.name === 'ruleNotificationTime' ? 'immediate' : '';
  ...
  function syncFollowUpMode(value){
    ...
    if(field){field.classList.toggle('hidden',value === 'no');}
  }
  ...
  function renderRuleNotificationCards(){
    ...
    container.innerHTML = selected.length ? selected.map(channel => {
      const entries = ruleNotificationTemplateEntries(channel);
      return `<div class="rule-notification-card" data-rule-channel="${escapeHtml(channel)}"><div class="rule-notification-card-title">通知方式：${escapeHtml(channel)}</div><div class="rule-notification-card-grid"><label class="rule-field rule-field-wide"><span class="rule-field-label">通知模板</span>${ruleNotificationTemplateSelectMarkup(channel,entries)}</label><div class="rule-template-preview hidden"><span class="rule-template-preview-label">模板内容</span><div class="rule-template-preview-content"></div></div></div></div>`;
  ...
  function syncScopeMode(mode){
    ...
    const uploadVisible = mode === 'custom';
  ...
  function saveRule(){
    ...
    if(!name || !type){
      const target = !type ? document.getElementById('ruleCreateType') : document.getElementById('ruleCreateName');
      if(target){target.classList.add('rule-invalid');target.setAttribute('aria-invalid','true');setTimeout(()=>target.classList.remove('rule-invalid'),1400);}
      return;
    }
    ...
    ruleRecords.unshift({id:`RULE${new Date().toISOString().slice(0,10).replace(/-/g,'')}${String(ruleRecords.length+1).padStart(4,'0')}`,name,type,status:'待生效',effectiveAt:effectiveMonth ? `${effectiveMonth}-01` : immediateDate,thresholds:selectValue('ruleCreateReminder') === '阈值提醒' ? ruleThresholdReminderValues.slice() : [],notificationTargetIds:ruleSharedNotificationTargets.slice(),notificationTime:ruleNotificationTime === 'scheduled' ? textValue('ruleNotificationTimeInput') : 'immediate',createdAt:'2026-09-19 10:00:00',updatedAt:'2026-09-19 10:00:00',operator:'当前用户'});
    resetCreatePage();
